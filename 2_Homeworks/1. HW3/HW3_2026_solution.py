...
class CouponDispenser:
    ...
    Invariant:
      - len(customer_roster) == len(issued_indices)
      - issued_indices[k] is the index of the coupon for customer_roster[k]
    """
    ...
    def __init__(self, coupon_cards):
        ...
        Args:
            coupons (list[str]): list of possible coupons users can receive.
        """
    ...
    def issue_coupon(self, name):
        """
        Assign (or re-report) a coupon for `name` WITHOUT using dictionaries.

        See HW spec for full behavior.
        """
        ...
        # Re-report existing name
        if name in self.customer_roster:
            pos = self.customer_roster.index(name)
            coupon = self.coupon_cards[self.issued_indices[pos]]
            return f"That name already has a coupon: {coupon}"

        # Allow re-using the same coupon for different people:
        # pick from all coupons (duplicates across customers are OK).

        chosen_index = random.randrange(len(self.coupon_cards))
        self.customer_roster.append(name)
        self.issued_indices.append(chosen_index)
        return f"{self.coupon_cards[chosen_index]}"

    def distribute_session(self):
        """
        Control the interactive loop for the session.
        ...
        while True:
            ...
            if user_input == "show":
                ...
                for piece in pieces:
                    ...
                    if cleaned == "":
                        continue
                    print(self.issue_coupon(cleaned))
    ...
    def tally_distribution(self):
        """
        Extra credit:
        Print coupon distribution counts using ONLY lists.
        """
        if len(self.issued_indices) == 0:
            print("Empty")
            return

        frequency = [0] * len(self.coupon_cards)
        ...
        for i in range(len(self.coupon_cards)):
            ...
            count = frequency[i]
            line = f"{coupon} distribution count: {count}. "
            print(line)
        return
